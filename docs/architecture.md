...
The desktop layout uses three horizontal sections with a shared 16px spacing rhythm: preview, composer and gallery. A 56px header reduces top whitespace without shrinking connection controls. The main preview keeps a 16:9 frame, scales with viewport height between 180px and 340px, and retains enlarged playback. Desktop gallery thumbnails use 2:1 crops; their full video remains unchanged. The header names FLUX 3 camera control; connection status remains in the header rather than a duplicate footer link.
