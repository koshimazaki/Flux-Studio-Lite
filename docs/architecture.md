...
The desktop layout uses three horizontal sections with a shared 16px spacing rhythm: preview, composer and gallery. A 56px header reduces top whitespace without shrinking connection controls. The main preview matches the composer width, uses a 380px height limit, and retains enlarged playback. A 32px gap separates the composer from the gallery. The gallery can extend below the viewport; fitting every section on screen is not a layout constraint. Desktop gallery thumbnails use 2:1 crops; their full video remains unchanged. The header names FLUX 3 camera control; connection status remains in the header rather than a duplicate footer link.
