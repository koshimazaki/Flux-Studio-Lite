...
describe("local job API", () => {
  it("stops without a key, persists across reload and retains ownership and replay protection", async () => {
    const { post, base, cookie, bfl, store } = await setup();
    const { job } = await (
      await post("stop-fixture-key", input, { "x-byo-key": "visitor-test-key" })
    ).json();
    const endpoint = `${base}/api/jobs/${job.id}/stop`;
    expect((await fetch(endpoint, { method: "POST" })).status).toBe(404);
    const stopped = await (
      await fetch(endpoint, { method: "POST", headers: { cookie } })
    ).json();
    expect(stopped.job.status).toBe("stopped");
    expect(stopped.job.error).toContain("may still finish");
    expect(
      (
        await (
    ...
    expect(
      (
        await (
          await post("stop-fixture-key", input, {
            "x-byo-key": "visitor-test-key",
          })
        ).json()
      ).job.status,
    ).toBe("stopped");
    ...
    await store.load();
    expect(store.data.jobs.find((item) => item.id === job.id)?.status).toBe(
      "stopped",
    );
  });
  it("an in-flight poll cannot resurrect a stopped job", async () => {
    const { post, base, cookie, bfl, store } = await setup();
    const { job } = await (await post("stop-race-key")).json();
    let release!: () => void;
    let started!: () => void;
    ...
    bfl.poll = async () => {
      started();
      await gate;
      return { id: "provider-1", status: "Generating" };
    };
    const pending = fetch(`${base}/api/jobs/${job.id}`, {
      headers: { cookie },
    });
    await began;
    await fetch(`${base}/api/jobs/${job.id}/stop`, {
      method: "POST",
      headers: { cookie },
    });
    release();
    expect((await (await pending).json()).job.status).toBe("stopped");
    expect(store.data.jobs[0].status).toBe("stopped");
  });
  it("a late submission response cannot resurrect a stopped job", async () => {
    const { post, base, cookie, bfl, store } = await setup();
    let release!: () => void;
    let started!: () => void;
    const began = new Promise<void>((resolve) => {
    ...
    const pending = post("stop-submit-race");
    await began;
    await fetch(`${base}/api/jobs/${store.data.jobs[0].id}/stop`, {
      method: "POST",
      headers: { cookie },
    });
    release();
    expect((await (await pending).json()).job.status).toBe("stopped");
    expect(store.data.jobs[0].pollingUrl).toBeUndefined();
  });
